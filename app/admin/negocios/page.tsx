...
export default function NegociosCentral() {
  const [stats, setStats] = useState({ leads: 0, emAndamento: 0, concluidos: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      const supabase = createClient();
      const [{ count: leads }, { count: emAndamento }, { count: concluidos }] = await Promise.all([
      ...
      setStats({ leads: leads ?? 0, emAndamento: emAndamento ?? 0, concluidos: concluidos ?? 0 });
      setLoading(false);
    }
    load();
  }, []);
